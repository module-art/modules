...
<div class="card-body">
  @if($operation == 'create')
    <form id="the-form" method="POST" action="{{ route('type.store') }}" accept-charset="UTF-8">
  @else
    <form id="the-form" method="POST" action="{{ route('type.update', $type->id) }}" accept-charset="UTF-8">
      <input name="_method" type="hidden" value="PUT">
  @endif
    <input name="_token" type="hidden" value="{{ csrf_token() }}">
    <div class="form-group">
      <label for="content_type" class="control-label">Type de contenu (ex: article, événement...) :</label>
      <input class="form-control{{ $errors->has('content_type') ? ' is-invalid' : '' }}" name="content_type" type="text" value="{{ $operation == 'create' ? old('content_type') : $type->content_type }}" id="content_type">
    ...
    <div class="form-row">
      <div class="col-2 col-form-label">
        <label class="default_filtre"> Filtre par défaut :</label>
      </div>
  ...
  @if($operation == 'create')
    <div class="d-flex justify-content-around">
      ...
      </form>{{-- shortcut form close in create mode --}}
    </div>
  @endif
</div>{{-- card-body --}}
