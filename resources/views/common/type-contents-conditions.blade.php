@if(isset($rubrique->type_contents))
  ...
  @endphp
  <div class="large-bloc type-contents row equal" data-content_type='{{ $type['content_type'] }}' data-filtre="{{ $type['default_filtre'] }}" data-desc="{{ $type['descendant'] }}">
...
@elseif(isset($type_content))
  ...
  @if(View::exists('themes.'.env('APP_THEME', 'module-art') . '.' . $context . '.type-content-'.$type_content->type['content_type']))
    @include('themes.'.env('APP_THEME', 'module-art') . '.' . $context . '.type-content-'.$type_content->type['content_type'], [$type_content])
  @else
