...
@if(isset($rubrique->type_contents))
  ...
  <div class="large-bloc type-contents" data-content_type='{{ $type['content_type'] }}' data-filtre="{{ $type['default_filtre'] }}" data-desc="{{ $type['descendant'] }}">
    @if(View::exists('themes.'.config('modules.theme') . '.' . $context . '.type-list-'.str_slug($type->content_type)))
      ...
      'results' => $module_control->getSortedTypeRubriques($type, $type->default_filtre, $type->descendant, $archive)
      ])
  @else
      ...
      @include('common.' . $context . '.type-contents', [
        ...
        'results' => $module_control->getSortedTypeRubriques($type, $type->default_filtre, $type->descendant, $archive, true)
      ])
    @endif
  </div>
