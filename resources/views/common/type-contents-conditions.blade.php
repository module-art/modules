@if(isset($rubrique->type_contents))
  ...
  <div class="large-bloc type-contents row equal justify-content-center" data-content_type='{{ $type['content_type'] }}' data-filtre="{{ $type['default_filtre'] }}" data-desc="{{ $type['descendant'] }}">
    @if(View::exists('themes.'.config('app.theme') . '.' . $context . '.type-list-'.$type->content_type))
      {{--next include redirect to the specific view if exists--}}
      @include('themes.'.config('app.theme') . '.' . $context . '.type-list-'.$type->content_type, [
...
@elseif(isset($type_content))
  ...
  @if(View::exists('themes.'.config('app.theme') . '.' . $context . '.type-content-'.$type_content->type['content_type']))
    @include('themes.'.config('app.theme') . '.' . $context . '.type-content-'.$type_content->type['content_type'], [$type_content])
  @else
